var systemPopup = function (type, message, myAction) {
    if ($('#mypopup-container').length > 0) {
        $('#mypopup-container').stop(true, true).fadeOut(150, function () {
            $(this).remove();
        });
    }
    ...
    switch (type) {
        case 'confirm':
            var confirmBtnHtml = '<input id="mypopup-button-yes" class="btn-green" type="button" value="Yes" />'
                +'<input id="mypopup-button-no" class="btn-red" type="button" value="No" />';
            $popupBtns.html(confirmBtnHtml);
            break;
        ...
        default:
            var okBtnHtml = '<input id="mypopup-button-ok" type="button" value="Ok" />';
            $popupBtns.html(okBtnHtml);
    }
...
var scrollToTopFx = function () {
    var $mainNavigation = $('#main-navigation')
        ,navBottom = parseInt($mainNavigation.offset().top)
            + parseInt($mainNavigation.height())
            + (parseInt($mainNavigation.css('margin-bottom')) * 2)
            + (parseInt($mainNavigation.css('padding-bottom')) * 2)
            + (parseInt($mainNavigation.css('border-bottom-width')) * 2);

    $(window).scroll(function () {
        var windowScrollPosition = parseInt($(window).scrollTop())
            ,scrollerHtml = '<div id="scroller-to-top">^</div>';

        if (windowScrollPosition > navBottom) {
            if ($('#scroller-to-top').length < 1) {
                $('body').append(scrollerHtml);
            ...
            if ($('#scroller-to-top').length > 0) {
                $('#scroller-to-top').fadeOut(150, function () {
                    $(this).remove();
                });
            }
        }
...
var embedSearchFx = function (userOptions) {
    var options = $.extend({
        'url': null
        ,'o_label': null
        ,'o_id': null
                            ...
                            switch (pageNo) {
                                case '...':
                                    ...
                                    if (ellipsisType == 'ellipsis-prev') {
                                        ellipsisTargetPage = parseInt(parseInt(currentPage)
