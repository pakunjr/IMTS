...
class view_itemPackages {
    ...
    public function renderSearchForm ($keyword) {
        ...
        $output = $f->openForm(array('id'=>'', 'action'=>URL_BASE.'inventory_packages/search_package/', 'method'=>'post', 'enctype'=>'multipart/form-data')).$f->text(array('id'=>'search-keyword', 'value'=>$keyword, 'label'=>'Search')).$f->submit(array('value'=>'Search')).$f->closeForm().'<hr />';
        return $output;
    }
